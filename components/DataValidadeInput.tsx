    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-900/60 dark:bg-slate-950/80 backdrop-blur-sm">
      ...
      <div className="relative z-10 w-full max-w-sm bg-white dark:bg-slate-900 rounded-2xl shadow-2xl border border-slate-200 dark:border-slate-700 overflow-hidden animate-scale-in m-4">
        ...
        <div className="px-5 pt-5 pb-4">
          <div className="flex items-center gap-3">
            <div className="w-10 h-10 rounded-xl bg-slate-100 dark:bg-slate-800 flex items-center justify-center flex-shrink-0">
              <svg className="w-5 h-5 text-slate-600 dark:text-slate-450" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>
        ...
        <div className="px-5 pb-4">
          <div className="bg-slate-50 dark:bg-slate-800 border border-slate-100 dark:border-slate-800 rounded-xl px-4 py-3 flex items-center justify-between">
            <span className="text-[10px] font-semibold text-slate-400 dark:text-slate-500 uppercase tracking-wider">EAN</span>
            <span className="font-mono text-sm font-bold text-slate-800 dark:text-slate-200 tracking-wider">{ean}</span>
          </div>
        </div>

        {/* Date Inputs */}
        <div className="px-5 pb-4">
          <label className="block text-[10px] font-semibold text-slate-500 dark:text-slate-400 uppercase tracking-wider mb-2.5">
            Data de Vencimento
          </label>
          ...
          <div className="flex items-center justify-center gap-1 sm:gap-2">
            ...
            <div className="flex-1 max-w-[80px]">
              <input
                ...
                className="w-full h-14 text-center text-2xl font-bold font-mono text-slate-900 dark:text-slate-100 bg-slate-50/50 dark:bg-slate-800 border border-slate-200 dark:border-slate-700/80 rounded-xl placeholder:text-slate-400 dark:placeholder:text-slate-500 focus:outline-none focus:border-slate-400 dark:focus:border-slate-600 focus:ring-4 focus:ring-slate-500/5 transition-all duration-200"
                maxLength={2}
                aria-label="Dia"
              />
              <span className="block text-center text-[10px] text-slate-400 dark:text-slate-500 mt-1.5 font-medium uppercase tracking-wider">Dia</span>
            </div>

            {/* Separator */}
            <div className="flex items-center justify-center h-14 pb-4">
              <span className="text-2xl font-bold text-slate-350 dark:text-slate-700">/</span>
            </div>
            ...
            <div className="flex-1 max-w-[80px]">
              <input
                ...
                aria-label="Mês"
              />
              <span className="block text-center text-[10px] text-slate-400 dark:text-slate-500 mt-1.5 font-medium uppercase tracking-wider">Mês</span>
            </div>
            ...
            <div className="flex-[1.5] max-w-[110px]">
              <input
                ...
                aria-label="Ano"
              />
              <span className="block text-center text-[10px] text-slate-400 dark:text-slate-500 mt-1.5 font-medium uppercase tracking-wider">Ano</span>
            </div>
          </div>

          {/* Erro */}
          {erro && (
            <div className="mt-3.5 flex items-start gap-2.5 bg-rose-50/50 dark:bg-rose-950/10 border border-rose-100/80 dark:border-rose-900/30 rounded-xl px-3 py-2.5">
              ...
              <span className="text-xs font-medium text-rose-700 dark:text-rose-400 leading-normal">{erro}</span>
            </div>
          )}
        </div>
        ...
        <div className="px-5 pb-5 pt-1 flex flex-col gap-2">
          <button
            ...
            className="btn-success w-full h-12 text-sm font-semibold uppercase tracking-wider"
          >
          ...
          <button
            onClick={onCancel}
            className="btn-secondary w-full h-10 text-xs font-semibold uppercase tracking-wider"
          >
